...
class LOSNet(nn.Module):
    ...
    def __init__(
        ...
        self.spfc = nn.Sequential(
            nn.Linear(in_features=static_input_size, out_features=25),
            nn.LayerNorm(25),
            nn.ReLU(),

            nn.Linear(in_features=25, out_features=35, bias=True),
            nn.LayerNorm(35),
            nn.ReLU(),
            nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=hidden_size + 35, out_features=30, bias=True),
            nn.LayerNorm(30),
            nn.ReLU(),
